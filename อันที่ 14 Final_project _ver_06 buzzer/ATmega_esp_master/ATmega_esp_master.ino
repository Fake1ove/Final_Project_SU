...
unsigned long lastSendTime = 0;  // บันทึกเวลาส่งคำสั่ง
bool responseReceived = false;   // ตรวจสอบว่ามีข้อมูลตอบกลับหรือไม่

// โครงสร้างข้อมูลสำหรับเซ็นเซอร์ DHT11
typedef struct struct_message {
  float temperatureDHT11;  // อุณหภูมิจาก DHT11
...
void OnDataRecv(uint8_t *mac, uint8_t *incomingData, uint8_t len) {
   responseReceived = true; // มีข้อมูลตอบกลับ
  if (len == sizeof(struct_message)) { 
    // รับค่าจาก DHT11
    memcpy(&sensorData, incomingData, sizeof(sensorData)); 
    Serial.print("DHT11 : Temperature : ");
    Serial.print(sensorData.temperatureDHT11, 2); // แสดงอุณหภูมิเป็นทศนิยม 2 ตำแหน่ง
    Serial.print(" °C ");
...
void loop() {
  while (Serial.available()) {
    ...
    if (inChar == '\n') { // เมื่อข้อความครบแล้ว (จบที่ '\n')
      ...
      lastSendTime = millis(); // บันทึกเวลาส่งข้อมูล
      responseReceived = false; // รีเซ็ตค่า
  ...
  // ตรวจสอบว่าครบ 15 วินาทีแล้วยังไม่ได้รับข้อมูลตอบกลับหรือไม่
  if (!responseReceived && millis() - lastSendTime > 15000) {
    Serial.println("TimeOut");
    responseReceived = true;         // ป้องกันการทำงานซ้ำ
  }
  delay(1000);
}
