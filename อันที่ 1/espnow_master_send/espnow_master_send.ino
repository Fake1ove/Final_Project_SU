...
uint8_t receiverMac1[] = {0x2c, 0x3a, 0xe8, 0x25, 0xdb, 0x4c}; // ตัวรับที่ 1
uint8_t receiverMac2[] = {0x5c, 0xcf, 0x7f, 0x06, 0xf4, 0xc2}; // ตัวรับที่ 2
...
void loop() {
  while (Serial.available()) {
    ...
    Serial.println(inChar);  // แสดงข้อมูลที่รับจาก Serial Monitor

    if ((millis() - lastTime) > timerDelay) {  // ตรวจสอบเวลา
      if (inChar == '1') {
        // เปิดอุปกรณ์
        test.x = 1;
        test.y = 1;
